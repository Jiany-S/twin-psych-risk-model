...
def preprocess_dataframe(cfg: Mapping[str, object], df: pd.DataFrame, schema: DataSchema) -> pd.DataFrame:
    ...
    frame[schema.timestamp] = pd.to_numeric(frame[schema.timestamp], errors="coerce").fillna(0.0)
    frame[schema.time_idx] = frame[schema.time_idx].astype(int)
    frame[schema.stress_target] = frame[schema.stress_target].astype(float).clip(0.0, 1.0)
    frame[schema.comfort_target] = frame[schema.comfort_target].astype(float).clip(0.0, 1.0)
    ...
    for col in schema.robot_context:
        if col not in frame.columns:
            frame[col] = 0.0
        frame[col] = pd.to_numeric(frame[col], errors="coerce")
    frame[schema.hazard_zone] = pd.to_numeric(frame[schema.hazard_zone], errors="coerce").fillna(0).astype(int)
    ...
    frame[schema.specialization_col] = pd.to_numeric(frame[schema.specialization_col], errors="coerce").fillna(0).astype(int)
    frame[schema.experience_col] = pd.to_numeric(frame[schema.experience_col], errors="coerce").fillna(1).astype(int)
