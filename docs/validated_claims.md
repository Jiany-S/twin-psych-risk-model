...
| Real-model physiological replay can consume saved held-out prediction artifacts. | Engineering/scientific bridge | `streaming_real_model_physiological_replay_*` runs | Policy thresholds may emit no warnings even when ranking metrics are nonzero. |
