...
7. Run saved-prediction physiological replay: `python -m src.streaming.replay --config src/config/streaming_physiological.yaml`.
...
Ruff is configured in `pyproject.toml`; run `ruff check src scripts tests` when Ruff is installed.
